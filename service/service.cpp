...
Adapter::Adapter(const IP_ADAPTER_ADDRESSES& aa) : aa_(aa)
{
    ...
    const struct sockaddr_in* gwsa = reinterpret_cast<const struct sockaddr_in*>(
            (gwa != nullptr) ? gwa->Address.lpSockaddr : nullptr);
...
static std::string ipv4_ntoa(uint32_t a)
{
    sockaddr_in sa;
...
std::string Adapter::str() const
{
    ...
    std::ostringstream ss;
    ss << srcmac_str << " " << ipv4_ntoa(src_ip_) << " " << gwmac_str << " " << gwip_str
       << "\n     " << aa_.AdapterName << "\n     " << friendly << "\n     " << description;
    return ss.str();
}
...
class Ipv4Wrapper
{
  public:
    ...
    int sendto(
            pcap_t* p, const char* buf, int len, int flags, const struct sockaddr* to, int tolen);
...
class ServerCtx
{
  ...
  private:
    ...
    ssize_t rpc_sendto(
            int sock_id, const rpc_pcap_pktdata_t& bufv, int flags, const std::vector<char>& addrv);
    ...
    rpc_pcap_basic_ret_t rpc_pcap_open_live(
            const std::string& device, int snaplen, int promisc, int to_ms);
    rpc_pcap_compile_ret_t rpc_pcap_compile(
            int pcap_id, const std::string& filter_exp, int optimize, bpf_u_int32 netmask);
...
void ServerCtx::Run()
{
    ...
    svr_->bind("setsockopt",
            [this](int sock_id, int level, int optname, const std::vector<char>& opt) -> int {
                return rpc_setsockopt(sock_id, level, optname, opt);
            });
    svr_->bind("sendto",
            [this](int sock_id,
                    const rpc_pcap_pktdata_t& bufv,
                    int flags,
                    const std::vector<char>& addrv) -> ssize_t {
                return rpc_sendto(sock_id, bufv, flags, addrv);
            });
    svr_->bind("pcap_lib_version", [this]() -> std::string { return rpc_pcap_lib_version(); });
    svr_->bind("pcap_lookupdev",
            [this]() -> rpc_pcap_lookupdev_ret_t { return rpc_pcap_lookupdev(); });
    svr_->bind("pcap_open_live",
            [this](const std::string& device, int snaplen, int promisc, int to_ms)
                    -> rpc_pcap_basic_ret_t {
                return rpc_pcap_open_live(device, snaplen, promisc, to_ms);
            });
    svr_->bind("pcap_compile",
            [this](int pcap_id, const std::string& str, int optimize, bpf_u_int32 netmask)
                    -> rpc_pcap_compile_ret_t {
    ...
    svr_->bind(
            "pcap_freecode", [this](int filter_id) -> int { return rpc_pcap_freecode(filter_id); });
    svr_->bind("pcap_datalink", [this](int pcap_id) -> int { return rpc_pcap_datalink(pcap_id); });
    svr_->bind("pcap_close", [this](int pcap_id) -> int { return rpc_pcap_close(pcap_id); });
    svr_->bind("pcap_next_ex",
            [this](int pcap_id) -> rpc_pcap_next_ret_t { return rpc_pcap_next_ex(pcap_id); });
...
int ServerCtx::rpc_socket(int domain, int type, int protocol)
{
    ...
    LOG() << "socket(" << domain << " , " << type << " , " << protocol << ") = " << ret
          << " [sd:" << sd << "] (" << errorno_str(ret) << ")";
    return ret;
}

#ifdef WIN32
static int setsockopt_win32(
        socketd_t sd, int level, int optname, const char* optval, socklen_t optlen)
{
    int ret = -1;
...
int ServerCtx::rpc_setsockopt(int sock_id, int level, int optname, const std::vector<char>& opt)
{
    ...
    if (it != sm_.end()) {
        ...
        ret = (deets.domain_ != LX_AF_PACKET)
                      ? mysetsockopt(sd, level, optname, opt.data(), (socklen_t)opt.size())
                      : 0;
    }
    LOG() << "setsockopt(" << sock_id << " [sd: " << sd << "], " << level << ", " << optname << ", "
          << tohex_short(opt.data(), opt.size()) << " len: " << opt.size() << "]) = " << ret;
    return ret;
}
 ...
 * Sloppy hack for now:
 *
 * In theory we keep a map of pcap instances around for the
 * (nonexistent) case that multiple simultaneous clients
 * are doing operations. When we are sending a pcap_sendpacket()
 * we don't much care what instance does the job, only that it
 * is open and active.
 **/
ssize_t ServerCtx::rpc_sendto(
        int sock_id, const rpc_pcap_pktdata_t& bufv, int flags, std::vector<char>& addrv)
{
    ssize_t ret = -1;
    socketd_t sd = -1;
    ...
    if (it != sm_.end()) {
        ...
        LOG() << "sendto domain: " << deets.domain_ << " type: " << deets.type_ << "(" << AF_INET
              << ", " << SOCK_RAW << ")";
    ...
    LOG() << "rpc_sendto(" << sock_id << " [sd: " << sd << "], "
          << "[buf]"
          << ", " << length << ", \"" << sockaddr_to_str(sa) << "\", " << sa_len << ") = " << ret;
    return ret;
}

std::string ServerCtx::rpc_pcap_lib_version() { return pcap_lib_version(); }
...
rpc_pcap_basic_ret_t ServerCtx::rpc_pcap_open_live(
        const std::string& device, int snaplen, int promisc, int to_ms)
{
    LOG(3) << "pcap_open_live(" << device << ", " << snaplen << ", " << promisc << ", " << to_ms
           << ")";
    errbuf_t errbuf;
#ifdef WIN32
    std::string pcap_dev;
 ...
 **/
rpc_pcap_compile_ret_t ServerCtx::rpc_pcap_compile(
        int pcap_id, const std::string& filter_exp, int optimize, bpf_u_int32 netmask)
{
    // std::string fe = "icmp";
    ...
    LOG() << "rpc_pcap_compile(" << pcap_id << ", " << filter_exp << ", " << optimize << ", "
          << tohex_field(netmask) << ") = " << ret;
    int filter_id = -1;
    if (ret == 0) {
...
rpc_pcap_next_ret_t ServerCtx::rpc_pcap_next_ex(int pcap_id)
{
    ...
    if (it != pcm_.end() && (ret = pcap_next_ex(it->second, &pkthdr, &pktdata)) == 1) {
        ...
        rpc_pcap_pkthdr_t rpc_pkthdr =
                std::make_tuple(tv_sec, tv_usec, pkthdr->caplen, pkthdr->len);
...
static void logmsg(const std::string& msg) { std::cout << msg; }
