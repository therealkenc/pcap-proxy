...
public:
    ...
    int pcap_compile_nopcap(int snaplen, int linktype, struct bpf_program *fp, 
...
private:
    ...
    int rpc_setsockopt(const sockdeets_t &sd, int level, int optname, 
                    const void *optval, socklen_t optlen);
    ...
    ssize_t rpc_sendto(int sockfd, const void *buf, size_t len, int flags,
                      const struct sockaddr *dest_addr, socklen_t addrlen);
    pcap_t* rpc_pcap_open_live(const char *device, int snaplen,
            int promisc, int to_ms, char *errbuf);
    void prepare_dlfns();
};
