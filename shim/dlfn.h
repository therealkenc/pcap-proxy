...
typedef int (*fngetsockopt)(int sockfd, int level, int optname, void* optval, socklen_t* optlen);
...
typedef pcap_t* (*fnpcap_open_offline_with_tstamp_precision)(
        const char* fname, u_int precision, char* errbuf);
typedef pcap_t* (*fnpcap_fopen_offline)(FILE* fp, char* errbuf);
typedef pcap_t* (*fnpcap_fopen_offline_with_tstamp_precision)(
        FILE* fp, u_int precision, char* errbuf);
typedef pcap_t* (*fnpcap_open_live)(
        const char* device, int snaplen, int promisc, int to_ms, char* errbuf);
typedef pcap_t* (*fnpcap_open_dead)(int linktype, int snaplen);
typedef pcap_t* (*fnpcap_open_dead_with_tstamp_precision)(
        int linktype, int snaplen, u_int precision);
...
typedef int (*fnpcap_compile)(
        pcap_t* p, struct bpf_program* fp, const char* str, int optimize, bpf_u_int32 netmask);
typedef int (*fnpcap_compile_nopcap)(int snaplen, int linktype, struct bpf_program* fp, char* str,
...
typedef int (*fnpcap_lookupnet)(
        const char* device, bpf_u_int32* netp, bpf_u_int32* maskp, char* errbuf);
typedef int (*fnpcap_offline_filter)(
